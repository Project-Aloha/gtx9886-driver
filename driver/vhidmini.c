...
--*/
{
    ...
    UINT8 allBuf[MAX_POINT_NUM * BYTES_PER_COORD] = { 0 };
    ...
    default:
        // Get All Points Info
        RtlCopyMemory(allBuf, &preBuffer[2], 10);
        SpbDeviceWriteRead(pDevice, gtx9886_get_coor, &allBuf[10], sizeof(gtx9886_get_coor), BYTES_PER_COORD * (touch_count-1) + BYTES_CHKSUM);
#ifdef DEBUG
...
#define WA(...) {                                                                                           \
    SpbDeviceWrite(pDevice, (UINT8[]){ ##__VA_ARGS__ }, sizeof((UINT8[]){ ##__VA_ARGS__ }));  \
}
